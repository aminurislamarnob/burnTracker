...
- `NotificationManager` (`UNUserNotificationCenter`) fires an **edge-triggered** alert once when an account's 5-hour session utilization first crosses `alertThreshold`, re-arming only after it drops back below.
